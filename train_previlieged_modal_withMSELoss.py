...
#=========================================================================#
#当前版本教师网络采用三阶段训练，与此同时学生网络全阶段都是全参数可训练的#
#为了展示图像特权真正注入EEG网络中，添加MSE损失，让蒸馏模型对齐教师图像特征==>视觉重塑脑区#
#=========================================================================#
class PrivilegedTrainer:
    def __init__(self, config):
        ...
        # 🌟 路径 B 修改：历史记录字典新增 mse_align_losses
        self.history = {
            ...
            'train_mse_align_losses': [], 'val_mse_align_losses': [],  # <--- 新增记录
    ...
    def freeze_backbones(self):
        ...
        for p in self.model.student_network['feature_path'].parameters(): p.requires_grad = True
        print("🔒 冻结教师 Backbone，🔓 学生网络全参数可训练！")
    ...
    def unfreeze_backbone_last_layers(self, eeg_layers=4, img_layers=4):
        ...
        if self.model.student_modality == 'eeg':
            for p in self.model.student_network.parameters():
                p.requires_grad = True
        else:
            self._unfreeze_img_path(self.model.student_network['feature_path'], img_layers)
        print(f"🔓 解冻教师 Backbone 最后几层，学生保持全参可训")
    ...
    def _update_temperature(self):
        start_temp, end_temp, decay_rate = self.config.temperature, 1.0, 0.96
    ...
    def train_epoch(self, train_loader, optimizer):
        ...
        for left_img, right_img, left_eeg, right_eeg, labels in train_loader:
            ...
            # 🌟核心杀招：核爆级跨模态特征直接对齐
            # ==========================================
            s_feat = outputs['student_features']  # 学生网络当前提取的特征
            # 强制跨模态！让 EEG 学生去学 Image 老师的特征！
            t_feat = outputs['image_common_teacher']

            if s_feat is not None and t_feat is not None:
                # MSE 强对齐
                mse_loss = F.mse_loss(s_feat, t_feat.detach())
                loss_dict['mse_align_loss'] = mse_loss
                # 20.0 倍重锤惩罚
                loss_dict['total_loss'] = loss_dict['total_loss'] + 20.0 * mse_loss
            else:
                loss_dict['mse_align_loss'] = torch.tensor(0.0).to(self.device)
            # ==========================================
    ...
    def train_stage(self, stage_name, train_loader, val_loader, lr_head, lr_backbone, max_epochs, patience=5,
        ...
        for epoch in range(max_epochs):
            ...
            for split, metrics in [('train', train_metrics), ('val', val_metrics)]:
                ...
                self.history[f'{split}_mse_align_losses'].append(metrics['mse_align_loss'])  # 🌟 新增记录
            ...
            if (epoch + 1) % 5 == 0 or epoch == 0:
                print(
                    f"[{stage_name}] Epoch {epoch + 1}/{max_epochs} | Temp: {current_temp:.2f} | Lr: {current_lr:.2e}")
                print(f"  总损失: (Train: {train_metrics['total_loss']:.4f} / Val: {val_metrics['total_loss']:.4f})")
                print(
                    f"  🔥MSE 特征对齐 Loss: (Train: {train_metrics['mse_align_loss']:.4f} / Val: {val_metrics['mse_align_loss']:.4f})")
                print(
    ...
    def train(self):
        ...
        self.train_stage(
            ...
            train_loader=train_loader, val_loader=val_loader,
            lr_head=2e-4, lr_backbone=1e-4,  # 🌟 路径 B: 稍微放大点 backbone 学习率，让 MSE Loss 产生作用
    ...
    def _plot_training_results(self):
        ...
        fig, axes = plt.subplots(3, 2, figsize=(16, 15))  # 🌟 扩展绘图区域
        ...
        # 🌟 路径 B: 绘制新增的强对齐损失 MSE
        axes[2, 0].plot(epochs, self.history['train_mse_align_losses'], label='Train MSE Align Loss')
        axes[2, 0].plot(epochs, self.history['val_mse_align_losses'], label='Val MSE Align Loss')
        axes[2, 0].set_title('🔥 Hard MSE Feature Alignment Loss', fontweight='bold')
        axes[2, 0].legend()
