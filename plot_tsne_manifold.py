...
def plot_tsne_comparison(features_baseline, features_distilled, labels, save_path):
    ...
    sns.scatterplot(
        ...
        alpha=0.8, s=150, ax=axes[0], legend=False, edgecolor="w", linewidth=0.5
    )
    # Label 字体放大到 22，增加 padding 避免和刻度贴太紧
    axes[0].set_xlabel("t-SNE Dimension 1", fontsize=22, fontweight='bold', labelpad=15)
    axes[0].set_ylabel("t-SNE Dimension 2", fontsize=22, fontweight='bold', labelpad=15)
    axes[0].set_xticks([])
    axes[0].set_yticks([])
    # Title 字体放大到 26，为避开巨大的 xlabel，进一步下沉到 -0.32
    axes[0].set_title("(a) Pure EEG Model (w/o KD)", fontsize=26, fontweight='bold', y=-0.32)

    # ================= (b) Distilled =================
    sns.scatterplot(
        x=tsne_distilled[:, 0], y=tsne_distilled[:, 1],
        hue=labels, style=labels, markers=markers, palette=palette,
        alpha=0.8, s=150, ax=axes[1], edgecolor="w", linewidth=0.5
    )
    axes[1].set_xlabel("t-SNE Dimension 1", fontsize=22, fontweight='bold', labelpad=15)
    axes[1].set_ylabel("t-SNE Dimension 2", fontsize=22, fontweight='bold', labelpad=15)
    axes[1].set_xticks([])
    axes[1].set_yticks([])
    axes[1].set_title("(b) Distilled EEG Model (Online KD)", fontsize=26, fontweight='bold', y=-0.32)

    # 图例设置：图例内的标记(markersize=15)和文字(fontsize=18)也需要放大
    handles = [plt.Line2D([0], [0], marker=markers[i], color='w', markerfacecolor=palette[i], markersize=15) for i in
               [0, 1]]
    axes[1].legend(
        handles, class_names,
    ...
    # 适度留白底部，防止巨型 title 导出时被截断
    plt.subplots_adjust(bottom=0.28)

    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"🎉 巨型字号 t-SNE 图像已成功保存至: {save_path}")
